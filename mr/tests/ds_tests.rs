...
fn test_taskman_single_threaded() { 
    ...
    let task3 = Task::new(vec!(String::from("d"), String::from("f"), String::from("g")), TaskType::Map, TaskID::MapID(100));
    ...
    // added to taskman
    taskman.task_completed(String::from("d"), ReduceType::Expedited, 2, 2, 0).unwrap();
    ...
    assert_eq!(taskman.status(), MapReduceStatus::Completed);
}
...
async fn async_taskman_multithread_completion() { 
    ...
    while !completed {
        ...
        join.await.unwrap();
        {
            let task_ref= Arc::clone(&ts_taskman);
            let safe_taskman = task_ref.lock().unwrap();
            if safe_taskman.status() == MapReduceStatus::Completed {
                completed = true;
            }
        }
    };
