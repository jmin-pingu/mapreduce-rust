...
impl mr::rpc::TaskService for Coordinator {
    ...
    async fn completed_task(mut self, _: context::Context, task: String, reduce_type: ReduceType, nreduce: usize, nmap: usize, worker_id: i8){ 
...
pub async fn main() -> anyhow::Result<()> {
    ...
    listener
        ...
        .map(|channel| {
            let server = Coordinator{taskman: taskman.clone(), socket_addr: channel.transport().peer_addr().unwrap()};
